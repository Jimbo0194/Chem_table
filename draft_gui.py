...
class App(tk.Frame):
    ...
    def create_widgets(self):
        ...
        column1 = [('H', 'Hidrógeno'), ('Li', 'Litio'), ('Na', 'Sodio'), ('K', 'Potasio'), ('Rb', 'Rubidio'),
                   ('Cs', 'Cesio'), ('Fr', 'Francio'), ('Uue', 'Ununennio')]

        # create all tk.Buttons with a loop
        r = 2
        c = 0
        for b in column1:
            tk.Button(self, text=b[0], width=5, height=2, bg=self.colores[self.elementos[b[1]].tipo],
                      command=lambda text=b: self.info(text[1])).grid(row=r, column=c)
            r += 1
